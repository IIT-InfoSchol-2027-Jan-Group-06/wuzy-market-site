...
typography:
  # Three marketing voices, one in-app voice. Six font tokens alias to Anton.
...
`backdrop-filter` survives in exactly three places: the desktop nav pill
(`blur(20px)`), `.btn-glass-yellow` (`blur(16px) saturate(1.5)`), and the
waitlist input and success panel (`blur(24px)`).
...
Six of the seven `--font-*` tokens alias to Anton (`--font-serif` is not a serif;
...
- **Sub-copy** — `text-base font-normal leading-relaxed text-charcoal/70`,
  constrained to `max-w-md` (28rem) or `max-w-lg` (32rem).
...
- Pinned sections (`#features`, `#community`, `#why`) carry **no padding on the
...
A hard convention, three for three (`#features`, `#community`, `#why`):
...
One vocabulary for all four screens: three drift speeds (`heavy 12/6.8`,
...
4. Why Choose `max-width: 767px` — heading, padding and overlap shrink to fit a
   pinned viewport
...
- Keep one `SCREEN_UNITS = 390` ruler for all four screens. When each divided by
  its own export width (360 / 393 / 402), the same `290` nav rendered 12% wider
  on Feed than on Discover.

**Don't**

- **Don't add `scroll-behavior: smooth`.** Three sections are pinned with
...
- **Archivo Black is imported from Google Fonts and used by nothing.** A NavBar
  ...
  Anton.
- **24 local font files in `src/assets/fonts/` are unreferenced**, as are
...
6. **Did I check §12 Don'ts?** Most visual regressions here are one of those nine.
